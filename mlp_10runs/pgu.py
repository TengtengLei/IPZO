...
#PGUIntSTPeZO
class PGUPeZO:
    def __init__(self, dim, seed='0x000000001', num_rows=8, num_cols=48, num_quo_lfsr=16, bit_width=8, device=None):
        self.dim = dim
        self.device = device
        self.root = './poly/'
        os.makedirs(self.root, exist_ok=True)
        ...
        self.num_quo_lfsr = num_quo_lfsr
        self.num_cols = num_cols
        self.num_arrays = math.ceil(dim / (segments_per_lfsr * (num_rows * num_quo_lfsr + 1) * num_cols))
        self.lfsr_len = 36
    ...
    def _load_or_generate_primitives(self):
        ...
        if os.path.exists(cache_file):
            ...
            poly_gen = galois.primitive_polys(2, degree=self.lfsr_len)
            primitives = [next(poly_gen) for _ in range(10000)]  # 预生成足够多
            with open(cache_file, 'wb') as f:
                pickle.dump(primitives, f)
            return primitives  # 返回全部
    ...
    def _shift_segment(self):
        ...
        # 重塑为 [num_arrays, per_array]
        segments_reshaped = segments.reshape(self.num_arrays, -1)
        return segments_reshaped.reshape(-1)[:self.dim].to(self.output_dtype)

    def step(self):
        self._update()
        self.shift_amount = - (self.shift_amount + 1) % self.num_quo_lfsr
        # self.shift_amount = (self.shift_amount + 1) % (self.num_quo_lfsr * self.num_rows + 1)

"""
测算 Spikingformer (ImageNet) QKV 线性层输出的每时间步空间脉冲发放率。
...
运行方式：
    cd sparsity
    CUDA_VISIBLE_DEVICES=0 python measure_sparsity_imagenet.py
"""
import sys
import os
...
# ── 配置 ──────────────────────────────────────────────────────────
CHECKPOINT = os.path.join(os.path.dirname(__file__), '..', 'checkpoint-284.pth.tar')
VAL_DIR    = os.path.join(os.path.dirname(__file__), '..', 'imagenet', 'data', 'val_blurred')
DEVICE     = 'cuda:0'
BATCH_SIZE = 32   # ImageNet 224×224 显存占用比 CIFAR-10 大，先用 32
T_STEPS    = 4
NUM_BLOCKS = 8    # checkpoint-284: depths=8
# ─────────────────────────────────────────────────────────────────
...
def build_val_loader(val_dir, batch_size):
    ...
    print(f'[数据] 共 {len(dataset)} 张图, {len(dataset.classes)} 个类别')
    return torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, shuffle=False,
        num_workers=4, pin_memory=True)


def measure_sparsity(net, val_loader, device, T=4, num_blocks=8):
    target_layers = {}
    ...
    print(f'\n[推理] 在 ImageNet val set 上测算 QKV 输出脉冲发放率...')
    with torch.no_grad():
    ...
    for name, s in stats.items():
        results[name] = [
            s['nonzero'][t] / s['total'][t] for t in range(T)
        ]
    return results


def print_results(results, T=4, num_blocks=8):
    ...
    for name, rates in results.items():
        ...
        print(f'{name:<40}{rates_str}  {avg*100:>6.2f}%')
    ...
    for qkv in ['q_lif', 'k_lif', 'v_lif']:
        ...
        print(f'[{qkv} 各block均值]{"":>18}{rates_str}  {avg*100:>6.2f}%')
    ...
    print(f'[QKV 全局均值]{"":>26}{rates_str}  {overall*100:>6.2f}%')
...
def main():
    ...
    results    = measure_sparsity(net, val_loader, device, T=T_STEPS, num_blocks=NUM_BLOCKS)
    print_results(results, T=T_STEPS, num_blocks=NUM_BLOCKS)

    save_path = os.path.join(os.path.dirname(__file__), 'sparsity_imagenet_qkv.csv')

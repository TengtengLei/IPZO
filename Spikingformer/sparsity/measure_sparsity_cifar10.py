"""
测算 Spikingformer (CIFAR-10) QKV 线性层输出的每时间步空间脉冲发放率。

目标层：block[i].attn.q_lif / k_lif / v_lif 的输出
  - 这是 q_conv → BN → LIF 之后的二值脉冲张量
  - shape: (T, B, C, N)，T 未被合并，天然按时间步分开

定义：
    firing_rate[t] = 时间步 t 中输出脉冲为 1 的个数 / 输出总元素数
    在整个 val set 上取平均，T=4 则输出 4 个数。
...
import model  # cifar10/model.py

# ── 配置 ──────────────────────────────────────────────────────────
CHECKPOINT = os.path.join(os.path.dirname(__file__), '..', 'checkpoint-405.pth.tar')
...
def measure_sparsity(net, val_loader, device, T=4):
    """
    hook 挂在 q_lif / k_lif / v_lif 的输出端。
    这些 LIF 节点的输出 shape 是 (T, B, C, N)，T 未被合并。
    对每个时间步 t，统计：
        firing_rate[t] = 脉冲为 1 的个数 / 该时间步全部输出元素数
    在整个 val set 上累加后求平均。
    """
    target_layers = {}
    for i in range(4):
    ...
    for name, module in target_layers.items():
        def make_hook(n):
            def hook(mod, inp, out):
                # out: (T, B, C, N) — 二值脉冲，T 已分开
                spikes = out.detach().float()
    ...
    print('\n[推理] 在 CIFAR-10 val set 上测算 QKV 输出脉冲发放率...')
    with torch.no_grad():
...
def save_csv(results, save_path, T=4):
    ...
    with open(save_path, 'w', newline='') as f:
        writer = csv.writer(f)
        header = ['layer'] + [f't={t}' for t in range(T)] + ['mean']
...
def main():
    ...
    save_path = os.path.join(os.path.dirname(__file__), 'sparsity_cifar10_qkv.csv')

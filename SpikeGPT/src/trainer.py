...
# print('logging to wandb... (comment it if you do not have wandb)')
...
log_file = open(os.environ.get("MEZO_LOG", "wik8-0.01.txt"), "a")   # set MEZO_LOG per run so the 4 modes don't collide
...
class TrainerConfig:
    ...
    optimizer_type = 'adam'  # 'adam' or 'mezo'
    ...
    # ==================== MeZO ====================
    mezo_random_mode = 'PGUXoR'
    mezo_learning_rate = 1e-4
    mezo_epsilon = 1e-3
    mezo_perturb_nums = 1
...
class Trainer:
    ...
    def __init__(self, model, train_dataset, valid_dataset, test_dataset, config):
        ...
        elif self.optimizer_type == 'mezo':
            from src.mezo import MeZO
            print(f"MeZO: random_mode={config.mezo_random_mode} "
                  f"perturb_nums={config.mezo_perturb_nums} "
                  f"lr={config.mezo_learning_rate} "
                  f"epsilon={config.mezo_epsilon}")
            steps_per_epoch = math.ceil(len(self.train_dataset) / config.batch_size)
            total_steps = config.max_epochs * steps_per_epoch
            self.optimizer = MeZO(
                model=self.model,
                device=self.device,
                random_mode=config.mezo_random_mode,
                lr=config.mezo_learning_rate,
                epsilon=config.mezo_epsilon,
                perturb_nums=config.mezo_perturb_nums,
                seed=config.mezo_seed,
                lr_schedule=getattr(config, 'mezo_lr_schedule', 'constant'),
                lr_final=getattr(config, 'mezo_lr_final', 0.0),
    ...
    def train(self):
        ...
        def run_epoch(split):
            ...
            for it, (x, y) in pbar:
                ...
                # y = y.to(self.device)
                if is_train:
                    if self.optimizer_type == 'mezo':
                        self.optimizer.step(x, y)
                        functional.reset_net(model)
                        loss = model(x, y)   # A's original per-batch clean forward at updated θ for the reported train loss
                        now_loss = loss.item()
                        # self.tokens += (y >= 0).sum()
                        lr = self.optimizer.lr   # actual (possibly scheduled) lr
                        self.lr = lr
                    else:
        ...
        for epoch in range(config.max_epochs):
            ...
            log_file.flush()
#             run_epoch('valid')   # disabled: valid consumes np.random and shifts the training data stream (results drift from the no-valid baseline)
#             val_ppl = math.exp(min(self.dev_loss, 20))
#             log_file.write(f'{epoch + 1} {self.avg_loss:.6f} {math.exp(self.avg_loss):.4f} {self.dev_loss:.6f} {val_ppl:.4f}\n')
#             log_file.flush()
#             run_epoch('valid')
#             log_file.write(
...
#             log_file.flush()
            ...
            if self.config.epoch_save_frequency > 0 and (epoch + 1) % self.config.epoch_save_frequency == 0:
                # keep ONE file per run: overwrite model_best.pth only when the (train) loss improves
                if self.avg_loss < self.min_dev_loss:
                    self.min_dev_loss = self.avg_loss
                    accelerator.wait_for_everyone()
                    unwrapped_model = accelerator.unwrap_model(model)
                    raw_model = unwrapped_model.module if hasattr(unwrapped_model, "module") else unwrapped_model
                    torch.save(raw_model.state_dict(), os.path.join(self.config.epoch_save_path, "model_best.pth"))
